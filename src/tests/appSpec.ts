...
it('/Api/images with invaild name', async () => {
  const res = await request.get('/api/images?filename=zizo&width=500&height=200');
...
it('/Api/images with text width', async () => {
  const res = await request.get('/api/images?filename=fjord&width=zizo&height=200');
  expect(res.text).toEqual('"width and height must be numbers"');
});


it('/Api/images with less than 50 width', async () => {
  const res = await request.get('/api/images?filename=fjord&width=5&height=200');
  // console.log(res)
  expect(res.text).toEqual('"width and height must be greater than 50"');
});

it('/Api/images with less than 50 height', async () => {
  const res = await request.get('/api/images?filename=fjord&width=500&height=20');
  // console.log(res)
  expect(res.text).toEqual('"width and height must be greater than 50"');
});